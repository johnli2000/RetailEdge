...
type
  BuffType = Record
   ...
   PrinterPort: array [1..4] of integer;
  end;
  TPrintJobListForm3 = class(TDataModule)
  ...
  private { Private declarations }
    OrderNo, IDStr, TableNo, ServicePerson, OrderDate, Telephone, CustomerName: string;
    VIPNo, Persons, OrderKindFlag, PrinterPort: Integer;
    PrintAll, JobListDuplicated, DeliveryFlag, ConfirmedOrder,
    AddNewItem: boolean;
    DueTime: TDateTime;
    VIPName, VIPAddress, VIPTelephone, VIPState, OpName: string;
    Buff: array [1..500] of BuffType;
    RelatedPrinterPort: array [1..12] of boolean;
  public
    { Public declarations }
  end;

var
  PrintJobListForm3: TPrintJobListForm3;
...
procedure TPrintJobListForm3.InitBuff;
var
 I: integer;
begin
 for I := 1 to 500 do
  begin
   ...
   Buff[I].VoidReason := '';
   Buff[I].PrinterPort[1] := 0;
   Buff[I].PrinterPort[2] := 0;
   Buff[I].PrinterPort[3] := 0;
   Buff[I].PrinterPort[4] := 0;
   Buff[I].SpecialOrder := '';
   Buff[I].Ingredients := '';
  end;
 for I := 1 to 12 do RelatedPrinterPort[I] := False;
end;

procedure TPrintJobListForm3.InsertLine(RecordNo: integer);
var
 I: integer;
begin
...
function TPrintJobListForm3.FindIDNo(IDNo: integer): integer;
var
 I: integer;
begin
 I := 1;
...
function TPrintJobListForm3.PostDataToBuff: integer;
var
 I, K, IDNo: integer;
  ...
  begin
   ...
   while Not EOF do
    begin
     ...
     Buff[I].PrinterPort[1] := FieldByName('PrinterPort').AsInteger;
     Buff[I].PrinterPort[2] := FieldByName('PrinterPort1').AsInteger;
     Buff[I].PrinterPort[3] := FieldByName('PrinterPort2').AsInteger;
     Buff[I].PrinterPort[4] := FieldByName('PrinterPort3').AsInteger;
     if Buff[I].PrinterPort[1] >= 1 then RelatedPrinterPort[Buff[I].PrinterPort[1]] := True;
     if Buff[I].PrinterPort[2] >= 1 then RelatedPrinterPort[Buff[I].PrinterPort[2]] := True;
     if Buff[I].PrinterPort[3] >= 1 then RelatedPrinterPort[Buff[I].PrinterPort[3]] := True;
     if Buff[I].PrinterPort[4] >= 1 then RelatedPrinterPort[Buff[I].PrinterPort[4]] := True;
     Next;
    end;
   Close;
  end;
    ...
    begin
     ...
     if K > 0 then
        begin
         ...
         Buff[K + 1].PrinterPort[1] := FieldByName('PrinterPort').AsInteger;
         Buff[K + 1].PrinterPort[2] := FieldByName('PrinterPort1').AsInteger;
         Buff[K + 1].PrinterPort[3] := FieldByName('PrinterPort2').AsInteger;
         Buff[K + 1].PrinterPort[4] := FieldByName('PrinterPort3').AsInteger;
         I := I + 1;
        end;
     Next;
    end;
...
procedure TPrintJobListForm3.OpenQuery(SQLStr: String);
begin
...
procedure TPrintJobListForm3.OpenInstructionQuery(SQLStr: String);
begin
 Screen.Cursor := crHourGlass;
 with InstructionQuery do
  begin
...
function TPrintJobListForm3.GetVoidReasonPro(VoidReasonCode: string): string;
var
 SQLStr: string;
begin
...
procedure TPrintJobListForm3.PrintOutPro(PrinterPort: integer);
var
 PrintStr, Description, StrTemp, LineStr: string;
 I, CharCount, PaperWidth, PageCounter, FontFlag: integer;
 Total: double;
begin
  ...
  begin
   ...
   if DueTime <> 0 then
      begin
       ...
       PrintStr := 'Pickup Date: ' + FormatDateTime('h:mmam/pm', DueTime);
       DataForm.OutPutToPrinter(PrintStr);
       if JobListFontSize = 0 then DataForm.SetBlackFontB else DataForm.SetBlackFontC;
      end;
   DataForm.OutPutToPrinter(LineStr);
        ...
        if Buff[I].Description1 = '----' then  // Print Line
           ...
           begin
            ...
            if Not Buff[I].Instruction then
               begin
                if (Buff[I].PrinterPort[1] = PrinterPort) or
                   (Buff[I].PrinterPort[2] = PrinterPort) or
                   (Buff[I].PrinterPort[3] = PrinterPort) then
                   FontFlag := 1
                  else
                   FontFlag := 0;
            ...
            if Buff[I].Qty < 0 then
               begin
                ...
                if Description = '' then Description := 'Void';
                if FontFlag = 0 then
                   Description := Format('%-40s', [Description])
                  else
              ...
              if Not Buff[I].Instruction then
                 begin
                  if FontFlag = 1 then
                     ...
                     begin
                      StrTemp := Copy(Description, CharCount + 1, 40);
                 ...
                 begin
                  if FontFlag = 1 then
                     ...
                     begin
                      StrTemp := Copy(Description, CharCount + 1, 38);
                      CharCount := CharCount + 38;
                     end;
                  PrintStr := Format('  %s', [StrTemp]);
                  if FontFlag = 0 then
                     PrintStr := Format('%-40s', [PrintStr])
            ...
            if PrintIngredientsOnJobList and (FontFlag = 1) and (Buff[I].Ingredients <> '') then
               begin
                CharCount := 1;
   ...
   DataForm.SetBlackFontA;
   PrintStr := 'Printed Time: ' + FormatDateTime('dd/mmm/yyyy  h:mmam/pm', Date + Time);
   DataForm.OutPutToPrinter(PrintStr);
   DataForm.SetBlackFontB;
   DataForm.OutPutToPrinter(' ');
   DataForm.OutPutToPrinter(' ');
...
procedure TPrintJobListForm3.PrintConfirmedOrder;
var
 SQLStr: string;
begin
 ...
 if PrintAll then
    SQLStr := 'Select OrderI.ItemCode, Description1, Description2, Condition, Qty, ' +
              ...
              'OrderI.Discount, VoidReason, SpecialOrder, Ingredients, ' +
              'PrinterPort, PrinterPort1, PrinterPort2, PrinterPort3, ' +
              'Multiple ' +
              ...
              'Null, 0, IDNo, 0, 0, Null, Null, Null, 0, 0, 0, 0, CAST(0 As Bit) ' +
              'From OrderI ' +
    ...
    SQLStr := 'Select OrderI.ItemCode, Description1, Description2, Condition, Qty, ' +
              ...
              'SubDescription2, SubDescription3, Seat, IDNo, JobListColor, ' +
              'OrderI.Discount, VoidReason, SpecialOrder, Ingredients, ' +
              'PrinterPort, PrinterPort1, PrinterPort2, PrinterPort3, Multiple ' +
              'From OrderI, MenuItem Left Join IngredientsTable ' +
              'On MenuItem.ItemCode=IngredientsTable.ItemCode ' +
              'Where MenuItem.ItemCode=OrderI.ItemCode and ' +
              'OrderNo=' + Chr(39) + CheckQuotes(OrderNo) + Chr(39) +
 ...
 if RelatedPrinterPort[PrinterPort] then
    begin
...
procedure TPrintJobListForm3.PrintUnConfirmedOrder;
var
 SQLStr: string;
begin
 AddNewItem := False;
 ...
 if PrintAll then
   ...
   SQLStr := 'Select HoldItem.ItemCode, Description1, Description2, Condition, Qty, ' +
             ...
             'On MenuItem.ItemCode=IngredientsTable.ItemCode ' +
             'Where MenuItem.ItemCode=HoldItem.ItemCode and ' +
             'HoldNo=' + OrderNo + ' and Condition > 0 and Printed = 0 Order By ID';
 OpenInstructionQuery(SQLStr);
 if PrintAll then
    SQLStr := 'Select HoldItem.ItemCode, Description1, Description2, ' +
              ...
              'SubDescriptionSwap, Null As VoidReason, SpecialOrder, Ingredients, ' +
              'PrinterPort, PrinterPort1, PrinterPort2, PrinterPort3, Multiple ' +
              'From HoldItem, MenuItem Left Join IngredientsTable ' +
              'On MenuItem.ItemCode=IngredientsTable.ItemCode ' +
              'Where MenuItem.ItemCode=HoldItem.ItemCode and ' +
              'Condition = 0 and HoldNo=' + OrderNo
  else
   SQLStr := 'Select HoldItem.ItemCode, Description1, Description2, ' +
             'Condition, Qty, HoldItem.Price, PriceSelect, SubDescription, ' +
             'SubDescription1, SubDescription2, SubDescription3, 0 As Seat, ' +
             ...
             'On MenuItem.ItemCode=IngredientsTable.ItemCode ' +
             'Where MenuItem.ItemCode=HoldItem.ItemCode and ' +
             'Condition = 0 and Printed = 0 and HoldNo=' + OrderNo;
 if Not PrintZeroQtyItemsOnJobList then SQLStr := SQLStr + ' and ABS(Qty) >= 0.01 ';
...
procedure TPrintJobListForm3.PrintJobListPro;
begin
 if ConfirmedOrder then
    PrintConfirmedOrder
   else
...
procedure TPrintJobListForm3.PrintJobList(OrderNo, IDStr: string; PrintAll, ConfirmedOrder: boolean; PrinterPort: integer);
begin
 Application.CreateForm(TPrintJobListForm3, PrintJobListForm3);
 PrintJobListForm3.PrintAll := PrintAll;
 PrintJobListForm3.OrderNo := OrderNo;
 PrintJobListForm3.IDStr := IDStr;
 PrintJobListForm3.ConfirmedOrder := ConfirmedOrder;
 PrintJobListForm3.PrinterPort := PrinterPort;
 PrintJobListForm3.PrintJobListPro;
 PrintJobListForm3.Free;
end;
