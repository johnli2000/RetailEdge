...
type
  BillBuffType = Record
   ItemCode, Description1, Description2, SubDescription, SubDescription1,
   SubDescription2, SubDescription3: string;
   Condition, PriceSelect: Integer;
   Qty, Price, TaxRate, Discount: double;
  end;
  TPrintOrderForm2 = class(TDataModule)
    Query: TADOQuery;
    procedure OpenQuery(SQLStr: String);
    procedure InitBillBuff;
    procedure ReadBillData;
    procedure PrintOutPro;
    procedure PrintOrderPro(OrderNo: string);
...
var
  PrintOrderForm2: TPrintOrderForm2;
...
procedure TPrintOrderForm2.OpenQuery(SQLStr: String);
begin
...
procedure TPrintOrderForm2.InitBillBuff;
var
 I: integer;
begin
 for I := 1 to 500 do
...
procedure TPrintOrderForm2.ReadBillData;
var
 SQLStr: string;
 I: integer;
begin
 InitBillBuff;
 SQLStr := 'Select OrderI.ItemCode, Sum(Qty) As Qty, OrderI.TaxRate, OrderI.Price, ' +
           'OrderI.Discount, Description1, Description2, Condition, PriceSelect, ' +
           'SubDescription, SubDescription1, SubDescription2, SubDescription3, ' +
           'Category.Code ' +
           'From OrderI, MenuItem, Category ' +
           'Where OrderI.ItemCode = MenuItem.ItemCode and MenuItem.Category = Category.Category and ' +
 ...
 SQLStr := SQLStr + ' Group By OrderI.ItemCode, OrderI.TaxRate, OrderI.Price, ' +
                    'OrderI.Discount, Description1, Description2, Condition, PriceSelect, ' +
                    'SubDescription, SubDescription1, SubDescription2, SubDescription3, ' +
                    'Category.Code Order By Category.Code';
 OpenQuery(SQLStr);
 I := 1;
 with Query do
  begin
...
procedure TPrintOrderForm2.PrintOutPro;
var
 TxtFile: TextFile;
...
begin
 ...
 DataForm.OutPutToPrinter(PrintStr);
 DataForm.OutPutToPrinter(LineStr);
 if (VIPNo >= 1) and (BillKind = sPHONEORDER) then
    begin
...
procedure TPrintOrderForm2.PrintOrderPro(OrderNo: string);
begin
 Application.CreateForm(TPrintOrderForm2, PrintOrderForm2);
 PrintOrderForm2.OrderNo := OrderNo;
 PrintOrderForm2.PrintOutPro;
 PrintOrderForm2.Free;
end;
