...
type
  TPrintOrderForm = class(TDataModule)
    Query: TADOQuery;
...
procedure TPrintOrderForm.OpenQuery(SQLStr: String);
begin
...
procedure TPrintOrderForm.PrintOutPro;
var
 TxtFile: TextFile;
 Description, PrintStr, StrTemp, FileName, FieldName, LineStr, Category, SQLStr: string;
 Amount, Qty, GST, DiscountRate, Discount, DollarDiscount, Price, RealPrice, TaxRate,
 ServiceCharge, OtherCharge, PaidAmount, Tips, Surcharge, TotalInvoice, CurrentGSTRate,
 RewardPoints, Balance: double;
 I, CharCount, PaperWidth, RoundFlag, VIPNo, BillKind, DiscountKind: integer;
 ...
 if Query.RecordCount <> 0 then
    begin
     while Not Query.EOF do
      begin
       if (PrintBillMode = 1) and (Category <> Query.FieldByName('Category').AsString) then
          begin
           Category := Query.FieldByName('Category').AsString;
           if PrintCategoryColor = 1 then DataForm.SetRedFontB
              else
               DataForm.SetBlackFontB;
           if (PrintOrderListDescription <> 2) or (Query.FieldByName('Category1').AsString = '') then
       ...
       if (DiscountKind <= 1) and (Query.FieldByName('Discount').AsFloat >= 0.01) then
          begin
           DiscountRate := Query.FieldByName('Discount').AsFloat / 100;
...
procedure TPrintOrderForm.PrintOrderPro(OrderNo: string);
begin
 Application.CreateForm(TPrintOrderForm, PrintOrderForm);
 PrintOrderForm.OrderNo := OrderNo;
 PrintOrderForm.PrintOutPro;
 PrintOrderForm.Free;
end;
